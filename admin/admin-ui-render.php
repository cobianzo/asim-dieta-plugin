...
function prefix_general_settings_section_callback() {
	echo '<p>' . __('A long description for the settings section goes here.', 'asim') . '</p>';
}
...
function prefix_general_settings_field_callback() {	
	...
	<fieldset>
		...
		<input type="checkbox" name="prefix_settings[setting_one]" id="prefix_settings[setting_one]" value="1" 
			...
			<label for="prefix_settings[setting_one]"><?php _e('Setting one', 'asim') ?></label>
			<br>
		...
		<input type="checkbox" name="prefix_settings[setting_two]" id="prefix_settings[setting_two]" value="1" 
			...
			<label for="prefix_settings[setting_two]"><?php _e('Setting two', 'asim') ?></label>
			<br>
		...
		<p class="description"><?php _e('Description of the text input field', 'asim'); ?></p>
...
function prefix_admin_interface_render () {
	...
	if ( isset( $_GET['settings-updated'] ) ) {
		...
		add_settings_error( 'prefix_settings_saved_message', 'prefix_settings_saved_message', __( 'Settings are Saved', 'asim' ), 'updated' );
	}
	...
	<div class="wrap">	
		...
		<form action="options.php" method="post">		
			...
			do_settings_sections( 'asim-dieta' );	// Page slug
			
			// Output save settings button
			submit_button( __('Save Settings', 'asim') );
			?>
		</form>
	</div>
	<?php
